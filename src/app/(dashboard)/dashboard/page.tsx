...
import { DataTable, type DataTableColumnDef } from "@/components/ui/DateTable";
...
export default function DashboardPage() {
  //reactive — the dashboard updates itself as invoices change
  const stats = useQuery(api.dashboard.stats);

  //the currency symbol used to be applied server-side; the query now returns
  //the raw number plus the user's currency, so format it here
  const data = {
    totalRevenue: stats
      ? `${currencyOption[stats.currency as TCurrencyKey] ?? ""}${stats.totalRevenue}`
      : "$0",
    totalInvoice: stats?.totalInvoice ?? 0,
    paidInvoice: stats?.paidInvoice ?? 0,
    UnpaidInvoice: stats?.UnpaidInvoice ?? 0,
    recentInvoice: stats?.recentInvoice ?? [],
    chartData: stats?.chartData ?? [],
  };

   const columns: DataTableColumnDef<Doc<"invoices">>[] = [
    {
